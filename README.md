...
- Browse your music library by songs, albums, artists, folders, and playlists
...
- Database management screen with library statistics, folder-based rescan with progress bar, and erase
...
- Playlist tab with browsing and instant playback
- Preferences screen with playback settings (auto-play after playlist select)
- About screen with app info and version
...
├── model/              Domain models (Song, Album, Artist, FolderItem, Playlist)
├── data/
│   ├── local/          Room database, entities, DAOs (songs, albums, artists, playlists, enc_music_lists)
│   └── repository/     MediaStore sync + Room queries + folder scanning + preferences
...
└── ui/
    ...
    └── screens/        Library, Player, Album, Artist, Database Management, Magic Search, Preferences, About screens
```
